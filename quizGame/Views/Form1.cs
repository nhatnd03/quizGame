...
namespace quizGame
{
    public partial class Form1 : Form
    {
        ...
        public Form1(FormLevel formLevel, FormIntroLevel formIntroLevel)
        {
            InitializeComponent();
            this.formLevel = formLevel; 
            this.formIntroLevel = formIntroLevel;
        ...
        private void UpdateProgressLabel(int  level)
        {
            if (level == 0)
            {
                lableCau.Text = $"Vòng khởi động\n{currentQuestionIndex + 1}/{GetQuestionsByLevel(level).Count}";
            }
            else if (level == 1)
            {
                lableCau.Text = $"Vòng tăng tốc\n{currentQuestionIndex + 1}/{GetQuestionsByLevel(level).Count}";
            }
            else if (level == 2)
            {

                lableCau.Text = $"Vòng vượt chướng ngại vật\n{currentQuestionIndex + 1}/{GetQuestionsByLevel(level).Count}";
            }
            else
            {
                lableCau.Text = $"Vòng về đích\n{currentQuestionIndex + 1}/{GetQuestionsByLevel(level).Count}";
                
            }
            lableCau.TextAlign = ContentAlignment.MiddleCenter;
        }
